...
$(document).ready(function() {
	...
	$('.mytable').dataTable({
		...
		"bAutoWidth": false,
		"aoColumnDefs": [
            { "sClass": "free-input", "aTargets": [ 0,1,3,4,6,7,9 ] },
            { "sClass": "select-box", "aTargets": [ 2 ] },
            { "sClass": "disabled", "aTargets": [ 5, 8,10,11 ] }
        ],
...
function showContextMenu(e) {
	...
	if (selCount > 1)
		$('#menu_delete_line','.media-table-menu').text("DZeile löschen");
	else
			...
			success: function(data,textStatus,jqXHR) {
				...
				if(colIndex>1){
					if (typeof data.mediaData.tkpGrossCHF !== 'undefined')
						$(e.target).parent().find("td:nth-child(6)").html(numberWithCommas(parseFloat(data.mediaData.tkpGrossCHF).toFixed(2)));
