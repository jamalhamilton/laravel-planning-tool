...
function calTotal() {
	...
	for (var i = 0; i < elems.length; i++) {
		...
		for (var j = 0 ; j < tableRows.length; j++) {
			...
			TableData = $(tableRows[j]).children("td")[5];
	...
	for (var i = 0; i < elems.length; i++) {
		...
		for (var j = 0 ; j < tableRows.length; j++) {
			...
			TableData = $(tableRows[j]).children("td")[9];
	...
	var cells = $("table.bigTable1780").find(".contentCollap  td:nth-child(10)");
	var sum = {};
	...
	for (var j = 0; j < cells.length; j++) {
		region = $(cells[j]).prev().prev().prev().prev().prev().prev().prev()[0].innerHTML;
		sum[region] += parseFloat($(cells[j])[0].innerHTML.replace("\'","").replace("\'","").replace("\'","").replace(",",""));
	}
...
$(document).ready(function() {
	...
	$('.mytable').dataTable({
		...
		"bAutoWidth": false,
		"aoColumnDefs": [
			{ "sClass": "free-input", "aTargets": [ 0,1,4,5,6,8 ] },
			{ "sClass": "select-box", "aTargets": [ 2 ] },
			{ "sClass": "disabled", "aTargets": [ 7,9 ] },
	...
	$('.btn-editrow').on('click', function() {
		...
		if(checkboxInfo) {
			infoHtml='<div style="width:19.8%;position: relative;"><span class="note ic-triangle"><div class="note-data">'+addInfo+'</div></span></div>';
		}
		$.ajax({
			headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
			type: 'POST',
	...
	$.ajax({
		...
		success: function(data,textStatus,jqXHR) {
			...
			$('.media-table-format').on('blur',function() {
				$(this).appendTo('body');$edit_target.removeClass('editing'); $edit_target.html( $(this).val() );
	...
	function tableRowDblClick(e) {
		...
		if ($edit_target.hasClass('free-input')) {
			...
			$editor.on('blur',function() {
				...
				reCalculation( $edit_target, value, 2, e);
			...
			$editor.on('keydown',function(event){
				...
				if ( event.which == 9 ) {
					...
					reCalculation( $edit_target, value, 1, e);
	...
	if (isValidNum) {
		$.ajax({
			...
			success: function(data,textStatus,jqXHR) {
				...
				if(colIndex>1){
					if(eventFlag == 1) {
						if (typeof data.mediaData.adPressureValue !== 'undefined' && colIndex != 4) {
							$(e.target).parent().find("td:nth-child(5)").html(numberWithCommas(parseInt(data.mediaData.adPressureValue)));
						}
						if (typeof data.mediaData.grossCHF !== 'undefined' && colIndex != 5) {
							$(e.target).parent().find("td:nth-child(6)").html(numberWithCommas(parseFloat(data.mediaData.grossCHF).toFixed(2)));
						}
					}
					if(eventFlag == 2) {
						if (typeof data.mediaData.adPressureValue !== 'undefined' && colIndex == 4) {
					...
					if (typeof data.mediaData.netCHF !== 'undefined')
						$(e.target).parent().find("td:nth-child(8)").html(numberWithCommas(parseFloat(data.mediaData.netCHF).toFixed(2)));
					if (typeof data.mediaData.nnCHF !== 'undefined')
						$(e.target).parent().find("td:nth-child(10)").html(numberWithCommas(parseFloat(data.mediaData.nnCHF).toFixed(2)));

					calTotal();

					//	$(e.target).parent().find("td:nth-child(6)").attr("class","disabled");
				}
